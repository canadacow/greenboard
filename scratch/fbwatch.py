"""Who writes to the CGA framebuffer? Use a MEM_WRITE hook on B8000-BBFFF and
report the top writer addresses. No guessing -- this is ground truth."""
...
mu.hook_add(UC_HOOK_MEM_WRITE,hook_mem,None,0xB8000,0xBC000)
...
def hc(uc,addr,size,user):
    ...
    if n>=nk[0]:
        nk[0]=n+3_000_000
        if pend[0] is None:
            pend[0]=keys[ki[0]] if ki[0]<len(keys) else ' '
            ki[0]+=1
...
print(f'instrs={ic[0]:,} t={time.time()-t0:.0f}s mode={mode[0]:#x}')
print('\n=== top framebuffer writers ===')
for (cs,ip),c in writers.most_common(15):
    lin=(cs*16+ip)&0xFFFFF
    try:
        i=next(md.disasm(bytes(mu.mem_read(lin,10)),lin),None)
