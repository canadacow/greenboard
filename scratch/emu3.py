"""Trace the boot sector step-by-step to find where it derails."""
import sys, struct
...
def hook_code(uc, addr, size, user):
    n[0] += 1
    if n[0] > 220: uc.emu_stop(); return
    cs, ip = uc.reg_read(UC_X86_REG_CS), uc.reg_read(UC_X86_REG_IP)
    try:
        ins = next(md.disasm(bytes(uc.mem_read(addr,10)), addr), None)
        t = f'{ins.mnemonic:<7s} {ins.op_str}' if ins else '??'
    except Exception: t = '??'
    ax=uc.reg_read(UC_X86_REG_AX); bx=uc.reg_read(UC_X86_REG_BX)
    cx=uc.reg_read(UC_X86_REG_CX); dx=uc.reg_read(UC_X86_REG_DX)
    es=uc.reg_read(UC_X86_REG_ES); ds=uc.reg_read(UC_X86_REG_DS)
    si=uc.reg_read(UC_X86_REG_SI); di=uc.reg_read(UC_X86_REG_DI)
    print(f'{n[0]:4d} {cs:04X}:{ip:04X} {t:<34s} ax={ax:04X} bx={bx:04X} cx={cx:04X} dx={dx:04X} ds={ds:04X} es={es:04X} si={si:04X} di={di:04X}')
mu.hook_add(UC_HOOK_CODE, hook_code)

def hook_intr(uc, intno, user):
    ax=uc.reg_read(UC_X86_REG_AX)
    print(f'      >>> INT {intno:02X}h ax={ax:04X} cx={uc.reg_read(UC_X86_REG_CX):04X} dx={uc.reg_read(UC_X86_REG_DX):04X} es={uc.reg_read(UC_X86_REG_ES):04X} bx={uc.reg_read(UC_X86_REG_BX):04X}')
    if intno == 0x13:
        ...
        if ah==0x02:
            ...
            print(f'          READ c={cyl} h={head} s={sec} n={al} file 0x{src:06x} -> {dest:05X}')
            uc.reg_write(UC_X86_REG_AX,0)
        uc.reg_write(UC_X86_REG_EFLAGS, uc.reg_read(UC_X86_REG_EFLAGS)&~1)
    elif intno==0x10 and ((ax>>8)&0xFF)==0x0F:
        uc.reg_write(UC_X86_REG_AX,0x0304)
