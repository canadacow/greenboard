...
IC_74LS32::IC_74LS32() : CallbackComponent("74LS32") { set_description("Quad 2-In OR"); }

void IC_74LS32::install(Socket& socket) {
...
void IC_74LS32::on_power_on() {
    update_outputs();
}

void IC_74LS32::on_power_off() {
    for (auto& g : gates_)
        g.y.release();
}

void IC_74LS32::on_signal_change(Fiber /*caller*/) {
    update_outputs();
}

void IC_74LS32::update_outputs() {
    for (auto& g : gates_) {
        bool any = g.a.level() == Level::High || g.b.level() == Level::High;
        g.y.drive(any ? Level::High : Level::Low);
    }
}
