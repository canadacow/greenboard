...
//   - The mouse is powered from the RTS/DTR lines. When the driver
//     toggles RTS low->high, the mouse resets and identifies itself by
//     transmitting 'M' (0x4D). This is how MOUSE.COM detects it.
//   - Movement/button events are 3-byte packets of 7-bit characters:
//       byte 1: 0 1 LB RB Y7 Y6 X7 X6   (bit 6 set = sync)
...
// baud), which naturally paces packets at the real ~40/s.
//
// Host input arrives via host_update() (atomics -- the renderer thread
// will call it; the UART drains on the clock thread). Nothing feeds it
// yet: this turn is the mouse itself, detectable and protocol-correct.
...
public:
    ...
    void host_update(int dx, int dy, bool left, bool right) {
        ...
        buttons_.store((left ? 1 : 0) | (right ? 2 : 0),
